...
elseif(isset($_POST['submitClient'])) {
  ...
  // If no errors so far, check address
  if ($error == '' && !empty($address)) {
    $conn = connectDB();

    $sql = "SELECT COUNT(*) as numAtAddress
            FROM client
            WHERE address = '{$address}'
              AND city = '{$city}' ";
    $result = runQueryForOne($conn, $sql)['numAtAddress'];
    if ($result > 0) {
      $error = "<p>There is already a client living at this address.</p>";
    }
